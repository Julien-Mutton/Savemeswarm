...
RESCUE_TEAM_PHONE_NUMBER = 'rescue_team_phone_number'
...
def send_alert(message):
    client.messages.create(
        ...
        to=RESCUE_TEAM_PHONE_NUMBER
    )


@app.route('/alert', methods=['POST'])
def alert():
    # Simulate receiving a message from the drone detection system
    data = request.json
    if data.get('danger_detected'):
        location = data.get('location')
        severity = data.get('severity', 'high')

        alert_message = f"Alert! Danger detected at location: {location}. Severity: {severity}."
        send_alert(alert_message)

        return jsonify({"status": "Alert sent", "message": alert_message}), 200
    return jsonify({"status": "No danger detected"}), 200
